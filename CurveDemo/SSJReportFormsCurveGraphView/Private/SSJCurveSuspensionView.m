...
@implementation SSJCurveSuspensionViewItem
...
@property (nonatomic, strong) SSJCurveSuspensionViewItem *item;
...
@interface SSJCurveSuspensionView () <UITableViewDataSource, UITableViewDelegate>

@property (nonatomic, strong) _SSJReportFormsCurveSuspensionTableView *tableView;

@end

@implementation SSJCurveSuspensionView
...
- (void)setUnitSpace:(CGFloat)unitSpace {
    if (_unitSpace != unitSpace) {
        ...
        _tableView.estimatedSectionHeaderHeight = unitSpace;
...
- (void)setItems:(NSArray<SSJCurveSuspensionViewItem *> *)items {
    _items = items;
    [_tableView reloadData];
    _tableView.contentOffset = CGPointMake(0, _contentOffsetX);
}
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
    if (_items.count > section) {
        SSJCurveSuspensionViewItem *item = _items[section];
