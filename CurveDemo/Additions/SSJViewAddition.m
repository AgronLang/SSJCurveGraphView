...
+ (void)load {
    SSJSwizzleSelector(self, @selector(layoutSubviews), @selector(SSJViewAddition_layoutSubviews));
}

- (void)SSJViewAddition_layoutSubviews {
    [self SSJViewAddition_layoutSubviews];
    UIActivityIndicatorView *indicator = objc_getAssociatedObject(self, kSSJLoadingIndicatorKey);
    if (indicator) {
        indicator.center = CGPointMake(self.width * 0.5, self.height * 0.5);
    }
}

- (void)ssj_showLoadingIndicator {
    [self ssj_showLoadingIndicatorWithColor:UIColor.whiteColor];
}

- (void)ssj_showLoadingIndicatorWithColor:(UIColor *)color {
...
@implementation UIView (SSJDashLine)
