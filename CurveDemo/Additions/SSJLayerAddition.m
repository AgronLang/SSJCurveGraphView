...
+ (void)load {
    SSJSwizzleSelector([self class], @selector(setBounds:), @selector(SSJLayerAddition_setBounds:));
}

- (void)SSJLayerAddition_setBounds:(CGRect)bounds {
    [self SSJLayerAddition_setBounds:bounds];
...
- (void)ssj_updateBorderLayerFrame {
    _SSJBorderLayer *layer = objc_getAssociatedObject(self, kBorderLayerKey);
    if (layer) {
        CGRect frame = UIEdgeInsetsInsetRect(self.bounds, [self ssj_borderInsets]);
